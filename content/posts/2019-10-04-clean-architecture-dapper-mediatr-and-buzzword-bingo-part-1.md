...
slug: clean-architecture-dapper-mediatr-and-buzzword-bingo-part-1
...
a `Brewery` and a `BeerStyle` enumeration so we can strongly type our families of beer. We've also initialized our
`Beers` list within our `Brewery` model - this is a great pattern to get into when working within the domain layer, as
consumers of our API should not have to worry about `NullReferenceExceptions` when interrogating logic based on a
...
![Project](/assets/blog/dotnet-mediatr/project-setup.png)
...
properies to our `Brewery.cs` entity to easily extract the number of beers a brewery has to offer. Let's add a
`BeerCount` property:
...
![Project_thus_far](/assets/blog/dotnet-mediatr/project-dtos-in-place.png)
...
![Domain_layer_finished](/assets/blog/dotnet-mediatr/finished-domain-layer.png)
