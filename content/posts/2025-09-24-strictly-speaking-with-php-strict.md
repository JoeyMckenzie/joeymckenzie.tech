...
Taking a play out of the pretty much any "clean code"-esque style book, imagine we're writing software to help brewers brew beer. Being a craft snob, I'm overdue for a good beer-based analogy. To brew a batch of the perfect programmer's IPA, we've got a few moving pieces to handle:
...
We've got five actors in `$kettle`, `$mashtun`, `$lauteringSystem`, `$chiller`, `$fermenter` each seemingly with their own dependencies and semantics. It would be much simpler if we could _just_ do something like:

```php
$brewery->brew($recipe);
```

That, more or less, is the facade pattern in a nutshell. You may often find it dressed up as a `BreweryManager`, `BreweryService`, `BreweryOrchestrator`, or some other enterprise-y noun. But it all means the same thing: make a complicated process simple.
