...
recently came across a scenario where I _really_ appreciated their power and flexibility while using Dapper to query for data that had a few one-to-many relationships going on. The root of the problem was not Dapper nor the relationships
...
of a SQL query that's gets us an aggregate view of all of a brewery's beers and employees, it might looks something like:
...
numbers of related entities. To capture all of the data from this query within a model, I might define an aggregate
...
-   Defines a closure for Dapper to callback to for each record we get back in the result
    ...
    -   On each pass, we'll also add the associated beer/employee that Dapper mapped out for us in the brewery aggregates list relationships (_foreshadowing intensifies_)
...
We've gotta few options here to combat this scenario to guarantee that we're only ever returned unique beers and employees
...
contract in place on our `Employee` model so we can use a `.Contains()` with the comparer. Implementing the `IEquatable<Employee>` contract would have our `Employee` now looking something like:
...
equality contract behind the scenes (take a look at [sharplab](https://sharplab.io) and play around with `class`/`record` types). What if we could make this code even _leaner_? Turns out, we can by using `HashSet`s. Let's update our `BreweryAggregate` to utilize those instead:
...
code for this post can be found [here](https://github.com/JoeyMckenzie/joeymckenzie.tech/tree/main/examples/dotnet/HopTracker) on my GitHub. As always, I hope someone out there finds this useful.
