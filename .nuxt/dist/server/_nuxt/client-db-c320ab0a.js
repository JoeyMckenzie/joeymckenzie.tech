...
function createPipelineFetcher(getContentsList) {
  ...
  const matchingPipelines = [
    ...
    (state, params) => {
      const filtered = state.result.filter((item) =>
        ensureArray(params.where).every((matchQuery) =>
          match(item, matchQuery),
        ),
      );
      return {
        ...state,
    ...
    function fetchSurround(state, params, db) {
      ...
      if (params.surround) {
        let _surround = surround(
          ((_a = state.result) == null ? void 0 : _a.length) === 1
            ? db
            : state.result,
          params.surround,
        );
  ...
  const transformingPiples = [
    ...
    function fetchDirConfig(state, params, db) {
      ...
      if (params.dirConfig) {
        const path =
          ((_a = state.result[0]) == null ? void 0 : _a._path) ||
          ((_c =
            (_b = params.where) == null ? void 0 : _b.find((w) => w._path)) ==
          null
            ? void 0
            : _c._path);
...
function createNav(contents, configs) {
  ...
  const pickNavigationFields = (content) => ({
    ...
    ...(isObject(content == null ? void 0 : content.navigation)
      ? content.navigation
      : {}),
  });
  const nav = contents
    ...
    .reduce((nav2, content) => {
      ...
      if (isIndex) {
        ...
        if (
          typeof (dirConfig == null ? void 0 : dirConfig.navigation) !==
            'undefined' &&
          !(dirConfig == null ? void 0 : dirConfig.navigation)
        ) {
          return nav2;
        }
        if (content._path !== '/') {
      ...
      const siblings = parts.slice(0, -1).reduce((nodes, part, i) => {
        ...
        if (
          typeof (conf == null ? void 0 : conf.navigation) !== 'undefined' &&
          !conf.navigation
        ) {
          return [];
        }
...
const withContentBase = (url) =>
  withBase(url, useRuntimeConfig().public.content.api.baseURL);
const contentStorage = prefixStorage(
...
function createDB(storage) {
  async function getItems() {
    ...
    if (previewToken) {
      const previewMeta = await storage
        .getItem(`${previewToken}$`)
        .then((data) => data || {});
      if (Array.isArray(previewMeta.ignoreSources)) {
        const sources = previewMeta.ignoreSources.map(
          (s) => `cache:${s.trim()}:`,
        );
      ...
      const previewContents = await Promise.all(
        previewKeys.map((key) => storage.getItem(key)),
      );
    ...
    const items = await Promise.all(
      Array.from(keys).map((key) => storage.getItem(key)),
    );
    return items;
  }
  return {
    ...
    query: (query) =>
      createQuery(createPipelineFetcherLegacy(getItems), {
        initialParams: query,
        legacy: true,
      }),
  };
...
async function initContentDatabase() {
  ...
  if (content.integrity !== +(integrity || 0)) {
    const { contents, navigation } = await $fetch(
      withContentBase(
        content.integrity ? `cache.${content.integrity}.json` : 'cache.json',
      ),
    );
    await Promise.all(
      contents.map((content2) =>
        _contentDatabase.storage.setItem(`cache:${content2._id}`, content2),
      ),
    );
...
async function generateNavigation(query) {
  ...
  if (
    !useContentPreview().getPreviewToken() &&
...
//# sourceMappingURL=client-db-c320ab0a.js.map
