...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 A MarkdownRenderer service converts markdown to HTML using CommonMark core, Phiki (TokyoNight), the Mermaid fenced-code extension, and Strikethrough; identical markdown yields identical HTML
- [ ] #2 ```mermaid fenced blocks render to <div class="mermaid">…</div> (drawn client-side), not to highlighted code
- [ ] #3 A posts:rerender artisan command re-renders content into content_html for all posts and is idempotent
- [ ] #4 Tests prove: a code block is Phiki-highlighted, a mermaid block becomes a mermaid div, and posts:rerender refreshes content_html for existing posts
