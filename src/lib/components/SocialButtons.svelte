...
<div
  class="mx-auto grid max-w-2xl grid-cols-1 gap-x-4 gap-y-4 py-8 sm:grid-cols-4"
>
  {#each socials as { display, href, icon, external }}
    <a
      class={`${buttonVariants({ variant: 'outline' })} flex flex-row gap-x-2`}
      {href}
    >
